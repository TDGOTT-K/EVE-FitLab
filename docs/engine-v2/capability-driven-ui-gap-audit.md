...
## CAP-10 补充：逐舱库存与折叠分区（2026-09-20）

用户要求每类船舱拥有独立空间，UI按实际舱室分区、分别展开折叠。当前0.210.6-ui.repair1的EveStaticInventory仍只有Cargo/Magazines/Crystals，UsedCubicMeters只计算普通货舱。UI现已把已知特殊舱容量从一行附注拆成独立分区；特殊舱明确显示“库存尚未接入”，不能视为该能力完成。

引擎后续交付要求：
- 公开supported holds目录：稳定holdId、显示名、装配后容量及来源trace、存在/未知/不支持状态；不能要求UI按船型穷举。
- 库存条目携带holdId，每舱分别返回used/capacity/remaining/withinCapacity、完整性和原因；普通货舱、晶体及弹仓口径不能重复计量。
- 提供逐舱物品准入及拒绝原因，覆盖舰队机库、舰船维护舱、燃料及矿石/冰矿/气云/弹药等特殊舱；维护舱的组装/打包舰船体积及舰内物品限制需验证后实现，UI不自行猜测。
- 公开编辑事务支持加入、移出、数量调整、跨舱移动和撤销；保存/导入/导出保留归属。未知舱不得静默落入普通货舱。
- MCP与CLI同输入验收：飞龙普通货舱1740、舰队机库50000、舰船维护舱5000000、燃料舱5000；单舱超载不污染其他舱，不同物品限制、无对应舱、跨舱失败原子性、晶体/弹仓不重复占用。

此项是独立库存能力缺口；本轮UI分区不实现新的游戏存储规则。
