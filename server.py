...
if os.environ.get('FITLAB_CALCULATOR','nengine') != 'nengine':
 raise ValueError('The legacy calculator has been removed; use NEngine.')
...
os.environ.setdefault('FITLAB_NENGINE_STATE',str(STATE/'native'))
 ...
 def do_GET(self):
  ...
  try:
   ...
   if self.path=='/api/engine-status':
    ...
    return self.reply({'provider':'nengine','status':bridge().discover()})
