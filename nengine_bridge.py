...
class NEngineBridge:
    def __init__(self, root=None, state=None):
        ...
        self.state = Path(state or os.environ.get('FITLAB_NENGINE_STATE',Path(__file__).resolve().parent / 'state/nengine-ui-local-r34')).resolve()
    ...
    def call(self, name, arguments=None):
        if name not in {'engine_status','catalog_search','catalog_item','catalog_type_details','catalog_variants','fit_analyze','mutation_rule','mutation_roll','booster_plan_analyze','booster_plan_roll','booster_plan_verify','capacitor_scenario'}:
