import {getLocale,localizeData,gameNameMarkup,gameLabelMarkup} from './i18n.js';
...
const number=v=>Number(v).toLocaleString(getLocale(),{maximumSignificantDigits:8,notation:v!==0&&(Math.abs(v)<0.0001||Math.abs(v)>=1e12)?'scientific':'standard'});
export function openAbyssalWorkbench({type,record,copy=false,onSave,api,options}){
 ...
 function paint(){
  $('.mutation-attributes').innerHTML=(rule?.attributes||[]).map(a=>{
   const id=a.attributeId,meta=metadata[id]||{},u=unit(a.unitId,meta.unit),fmt=x=>number(display(x,a.unitId))+(u?' '+u:''),label=localizeData(meta.labelNames)||meta.label||a.name;
 ...
 $('[data-close]').onclick=()=>{stop=true;dialog.close()};dialog.oncancel=e=>{if(busy)e.preventDefault();else stop=true};dialog.onclose=()=>{stop=true;window.removeEventListener('resize',layoutLabels);window.removeEventListener('fitlab-language-change',refreshLocale);picker.destroy();dialog.remove()};
 function layoutLabels(){for(const a of rule?.attributes||[])decorateTrack(a)}
 function refreshLocale(){const focus=document.activeElement,attribute=focus?.dataset.attr,bound=focus?.dataset.bound;paint();if(attribute)dialog.querySelector('[data-attr="'+attribute+'"]'+(bound?'[data-bound="'+bound+'"]':''))?.focus({preventScroll:true});}
 window.addEventListener('fitlab-language-change',refreshLocale);
