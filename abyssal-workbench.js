...
export function openAbyssalWorkbench({type,record,copy=false,onSave,api,options}){
 ...
 $('[data-close]').onclick=()=>dialog.close();dialog.oncancel=e=>{if(busy)e.preventDefault()};dialog.onclose=()=>{materialPicker.destroy();dialog.remove()};paint();dialog.showModal();if(receipt){busy=true;paint();api('mutation-rule',request()).then(r=>{metadata=r.metadata}).catch(e=>{$('.mutation-error').textContent=e.message}).finally(()=>{busy=false;paint()})}else loadRule();
}
