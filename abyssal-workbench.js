...
export function openAbyssalWorkbench({type,record,copy=false,onSave,api,options}){
 ...
 function updateMaterialIcon(){
  const material=options.find(o=>o.id===Number($('select').value)),icon=$('.mutation-plasmid-icon');
  icon.hidden=!material;
  if(material){icon.src='https://images.evetech.net/types/'+material.id+'/icon?size=64';icon.alt=material.name;}
 }
 updateMaterialIcon();
 function paint(){
  let good=0,bad=0;
 ...
 $('select').onchange=()=>{updateMaterialIcon();loadRule()};
