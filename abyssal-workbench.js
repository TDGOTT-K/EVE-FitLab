...
export function openAbyssalWorkbench({type,record,copy=false,onSave,api,options}){
 ...
 function controls(){
  ...
  dialog.querySelectorAll('[data-slider]').forEach(slider=>{const a=rule.attributes.find(a=>a.attributeId===+slider.dataset.slider),enabled=editing&&!busy&&!running&&a.minimumValue!==a.maximumValue;slider.setAttribute('aria-disabled',String(!enabled));slider.tabIndex=enabled?0:-1;});
  $('[data-apply]').hidden=!editing;
dialog.querySelectorAll('[data-apply],[data-roll],[data-save],[data-estimate],[data-trial],[data-targets] input,[data-targets] select').forEach(e=>e.disabled=busy||running||!rule);$('[data-save]').disabled=busy||running||!rule?.nativeInstanceSupported;picker.disabled=busy||running;$('[data-close]').disabled=busy;$('[data-stop]').hidden=!running;$('[data-stop]').disabled=stop||!running;$('[data-trial]').textContent=stream?'继续试验':'Roll 至合格';}
 ...
 function paint(){
  $('.mutation-attributes').innerHTML=(rule?.attributes||[]).map(a=>{
   const v=values[a.attributeId],meta=metadata[a.attributeId]||{},u=unit(a.unitId,meta.unit),fmt=x=>number(display(x,a.unitId))+(u?' '+u:'');
   ...
   return `<div class="mutation-edit-row"><label><b>${esc(meta.label||a.name)}</b><small>原值 ${esc(fmt(a.baseValue))} · 允许 ${esc(fmt(a.minimumValue))} ～ ${esc(fmt(a.maximumValue))}</small></label><div class="mutation-reading"><strong data-value="${a.attributeId}">${esc(number(display(v,a.unitId)))}</strong><span>${esc(u)}</span></div><small data-change>${comparison?.relativeChange!=null?(comparison.relativeChange>0?'+':'')+number(comparison.relativeChange*100)+'%':''}</small><div class="mutation-range" data-slider="${a.attributeId}" role="slider" aria-label="${esc(meta.label||a.name)}" aria-valuemin="${a.minimumValue}" aria-valuemax="${a.maximumValue}" aria-valuenow="${v}" aria-valuetext="${esc(fmt(v))}" style="grid-column:1/-1" title="虚线为原值；切换编辑模式可拖动"><i class="mutation-allowed" style="left:${position(a.minimumValue)}%;width:${position(a.maximumValue)-position(a.minimumValue)}%"></i><i class="mutation-baseline" style="left:${position(a.baseValue)}%"></i><i class="mutation-point" style="left:${position(v)}%"></i></div></div>`;
  }).join('');
  $('.mutation-summary').textContent=receipt?'随机生成'+(rolls?' · 本次试验 '+rolls+' 次':''):edit?'手动编辑 · 已通过引擎范围校验':busy?'正在读取规则…':'切换编辑模式可拖动范围条，或随机生成一次';
  ...
  dialog.querySelectorAll('[data-slider]').forEach(slider=>{
   const a=rule.attributes.find(a=>a.attributeId===+slider.dataset.slider),low=Math.min(a.minimumValue,a.baseValue),high=Math.max(a.maximumValue,a.baseValue);let pointer=null;
   const enabled=()=>editing&&!busy&&!running&&a.minimumValue!==a.maximumValue;
   const update=value=>{if(!enabled())return;value=Math.max(a.minimumValue,Math.min(a.maximumValue,value));if(value===values[a.attributeId])return;values[a.attributeId]=value;
    $('[data-value="'+a.attributeId+'"]').textContent=number(display(value,a.unitId));slider.querySelector('.mutation-point').style.left=100*(value-low)/(high-low||1)+'%';slider.setAttribute('aria-valuenow',String(value));slider.setAttribute('aria-valuetext',number(display(value,a.unitId))+' '+unit(a.unitId,metadata[a.attributeId]?.unit));invalidate();
   };
   const move=e=>{const rect=slider.getBoundingClientRect();if(!rect.width)return;const value=low+(high-low)*Math.max(0,Math.min(1,(e.clientX-rect.left)/rect.width)),span=a.maximumValue-a.minimumValue;update(a.minimumValue+span*Math.round(1000*Math.max(0,Math.min(1,(value-a.minimumValue)/span)))/1000);};
   slider.onpointerdown=e=>{if(!enabled()||e.button!==0)return;e.preventDefault();pointer=e.pointerId;slider.setPointerCapture(pointer);slider.focus({preventScroll:true});move(e);};
   ...
   slider.onkeydown=e=>{if(!enabled())return;const step=(a.maximumValue-a.minimumValue)/(e.shiftKey?100:1000),v=values[a.attributeId];const next={ArrowLeft:v-step,ArrowDown:v-step,ArrowRight:v+step,ArrowUp:v+step,Home:a.minimumValue,End:a.maximumValue}[e.key];if(next!==undefined){e.preventDefault();update(next)}};
  });
 ...
 dialog.querySelectorAll('[data-mode]').forEach(b=>b.onclick=()=>{if(busy||running)return;editing=b.dataset.mode==='edit';controls();});
