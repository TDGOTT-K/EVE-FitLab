export function openAfterHours(){
 ...
 const close=()=>{host.close();cleanup();};host.querySelector('button').onclick=close;
 const nav=()=>close();window.addEventListener('hashchange',nav);
 const cleanup=()=>{if(!host.isConnected)return;window.removeEventListener('hashchange',nav);host.querySelector('iframe').src='about:blank';host.remove();previous?.focus();};host.onclose=cleanup;
