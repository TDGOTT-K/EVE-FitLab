...
export function openScenarioEditor({fit,fits,shipName,calculate,onSave}){
 ...
 function draw(){
  ...
  async function speedLimit(){const request=++revision,id=form.elements.targetFitId.value;currentPlane.setSpeedLimit(null,id?'正在读取速度上限…':'右键空地放置目标');if(!id)return;
   try{const enemy=fits.find(f=>f.id===id);if(!enemy)throw Error('目标装配已删除，请重新选择');const [a,b]=await Promise.all([calculate(withoutScenario(fit)),calculate(withoutScenario(enemy))]);if(request!==revision||!dialog.open)return;if(!Number.isFinite(a.attributes.maxVelocity)||!Number.isFinite(b.attributes.maxVelocity))throw Error('速度数据缺失');currentPlane.setSpeedLimit(b.attributes.maxVelocity,'双方速度分别受各自舰船上限限制',a.attributes.maxVelocity);}
