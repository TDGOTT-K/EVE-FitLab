...
export function mountDpsChart(root,data,{mode='distance',onMode=()=>{}}={}){
 ...
 function render(){
  ...
  root.insertAdjacentHTML('beforeend','<div class="dps-chart-current"><span>当前 '+escapeHtml(fmtX(s.currentX,s))+'</span><b>'+escapeHtml(metricText(s.currentY,'DPS'))+'</b></div><svg class="dps-function-plot" viewBox="0 0 '+W+' '+H+'" role="img" aria-label="DPS 随'+s.label+'变化"><text x="'+L+'" y="11">DPS</text>'+grid+'<path class="dps-curve" d="'+path+'"/><line class="dps-current-line" x1="'+currentX+'" x2="'+currentX+'" y1="'+T+'" y2="'+B+'"/><circle class="dps-current-point" cx="'+currentX+'" cy="'+currentY+'" r="4"/><g class="dps-probe" visibility="hidden"><line y1="'+T+'" y2="'+B+'"/><circle r="3"/></g><text x="'+R+'" y="222" text-anchor="end">'+s.label+' / '+s.unit+'</text><rect class="dps-chart-hit" x="'+L+'" y="'+T+'" width="'+(R-L)+'" height="'+(B-T)+'"/></svg><div class="dps-chart-probe-value" aria-live="off">&nbsp;</div>');
  ...
  const note=document.createElement('div');note.className='dps-chart-fixed';note.textContent=fixed.join(' · ');root.append(note);
  const scope=document.createElement('div');scope.className='dps-chart-scope';scope.textContent='目标抗性后 · 不含换弹 · 其余条件固定';root.append(scope);
