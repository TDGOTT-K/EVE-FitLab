...
export function mountDpsChart(root,data,{mode='distance',onMode=()=>{}}={}){
 ...
 function render(){
  ...
  const currentMarkup=data.ideal||!Number.isFinite(s.currentY)?'': '<line class="dps-current-line" x1="'+currentX+'" x2="'+currentX+'" y1="'+T+'" y2="'+B+'"/><circle class="dps-current-point" cx="'+currentX+'" cy="'+currentY+'" r="4"/><text class="dps-probe-percent" x="'+(currentX>R-75?currentX-8:currentX+8)+'" y="'+(currentY<T+18?currentY+16:currentY-9)+'" text-anchor="'+(currentX>R-75?'end':'start')+'">'+(data.ratiosFromEngine?(Number.isFinite(s.currentRatio)?metricText(s.currentRatio*100,'%',1):''):(data.totalDps>0?metricText(s.currentY/data.totalDps*100,'%',1):''))+'</text>';
  root.insertAdjacentHTML('beforeend','<div class="dps-chart-current">'+(data.ideal?'<span>理想条件</span><b>采样峰值 '+escapeHtml(metricText(data.ratiosFromEngine?s.sampledPeak:Math.max(...s.points.map(p=>p[1])),damageUnit))+'</b>':'<span>当前 '+escapeHtml(fmtX(s.currentX,s))+'</span><b>'+escapeHtml(metricText(s.currentY,damageUnit))+'</b>')+'</div><svg class="dps-function-plot" viewBox="0 0 '+W+' '+H+'" role="img" aria-label="'+damageUnit+' 随'+s.label+'变化"><text x="'+L+'" y="11">'+damageUnit+'</text>'+grid+'<path class="dps-curve" d="'+path+'"/>'+currentMarkup+'<g class="dps-probe" visibility="hidden"><line y1="'+T+'" y2="'+B+'"/><circle r="3"/><text class="dps-probe-percent"/></g><text x="'+R+'" y="222" text-anchor="end">'+s.label+' / '+s.unit+'</text><rect class="dps-chart-hit" x="'+L+'" y="0" width="'+(R-L)+'" height="'+B+'"/></svg><div class="dps-chart-probe-value" aria-live="off">&nbsp;</div>');
  ...
  const scope=document.createElement('div');scope.className='dps-chart-scope';scope.textContent=data.scope||(data.ideal?'按当前配装与启用状态 · 不含换弹 · 参考半径非具体舰船实值':(data.attackMode==='edps'?'已扣目标抗性':'不扣目标抗性')+' · 不含换弹 · 其余条件固定');root.append(scope);
  ...
  hit.onpointermove=e=>{
   ...
   if(s.key==='angular'){
    ...
    const reference=data.angularLinearSpeedReferenceMeters??10000;speed.textContent=metricText(reference/1000,'km')+' 处横向速度 '+metricText(sample[0]*reference,'m/s',0);readout.append(speed);
   }
   ...
   label.textContent=data.ratiosFromEngine?(Number.isFinite(sample[2])?metricText(sample[2]*100,'%',1):''):(Number.isFinite(data.totalDps)&&data.totalDps>0?metricText(sample[1]/data.totalDps*100,'%',1):'');
