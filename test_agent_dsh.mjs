...
await connect({serverName:'fitlab',transport:'stdio',command:process.env.FITLAB_PYTHON||'python',args:[path.resolve('agent_mcp.py'),'--engine',root,'--state',path.join(state,'agent'),'--mcp-dll',dll],toolCallTimeoutMs:90000},async(call,tools)=>{
 assert.equal(tools.size,8);facadeSchemaChars=JSON.stringify([...tools.values()].map(t=>({name:t.name,description:t.description,parameters:t.parameters}))).length;
...
console.log(JSON.stringify({nativeCount,facadeToolCount:8,facadeSchemaChars,evidence},null,2));
