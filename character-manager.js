import {editPilotAvatar} from './pilot-avatar-editor.js';
import {pilotPortrait,copyDraftAvatar} from './pilot-portrait.js';
...
export function installCharacterManager({api,catalog,onReturn}){
 ...
 const addEntries=()=>[['新建自定义角色',newCharacter],['创建 I～IV 技能对比角色',createComparisonCharacters],['新建文件夹',()=>editFolder()],['从 EVE 官网导入',login]];
 ...
 async function showSkill(id){
  const type=skills.find(t=>t.id===id),detail=document.createElement('dialog');detail.className='character-action-dialog';
  detail.innerHTML='<div class="flow-head"><b>'+esc(type?.name||'技能说明')+'</b><button aria-label="关闭技能说明">×</button></div><div class="flow-body"><p role="status">正在读取官方技能说明…</p></div>';
  detail.querySelector('button').onclick=()=>detail.close();detail.onclose=()=>detail.remove();document.body.append(detail);detail.showModal();
  try{const data=await api('items/'+id);if(!detail.isConnected)return;const locale=getLocale(),text=data.description?.[locale]||data.description?.[locale==='zh-CN'?'zh':'en']||data.description?.en;
   const body=detail.querySelector('.flow-body');body.replaceChildren();const p=document.createElement('p');p.style.whiteSpace='pre-wrap';p.textContent=text?String(text).replace(/<br\s*\/?\s*>/gi,'\n').replace(/<[^>]*>/g,''):'暂无官方技能说明；不推算未提供的加成。';body.append(p);
   const note=document.createElement('small');note.textContent='官方技能说明；每级加成以原文为准，实际装配效果由引擎计算。';body.append(note);
 ...
 function drawSkills(){
  ...
  $('#character-skills').querySelectorAll('[data-skill]').forEach(el=>el.querySelectorAll('button').forEach(button=>button.onclick=()=>{
   ...
   const result=completeSkillPrerequisites(draft.skills,skills);draft.skills=result.skills;markDirty();drawSkills();if(result.raised.length)status('已按前置要求补齐 '+result.raised.length+' 项技能，可继续微调。');
  }));
 }
 ...
 function drawDetail(){
  ...
  $('#character-detail').innerHTML='<div class="character-skill-toolbar"><b id="character-skill-count"></b><button type="button" id="character-avatar" title="编辑头像" aria-label="编辑头像">'+pilotPortrait(c)+'</button>'+(draft?'<button id="skills-zero">全部未学习</button><span id="skill-presets"></span>':'')+'<span id="character-skill-points" title="'+esc(pointText.note(c))+'"><span>总技能点</span><strong>'+pointText(c)+'</strong><small>SP</small></span></div><div class="character-skill-filters"><input id="skill-search" aria-label="搜索技能" placeholder="搜索技能"><label><input id="skills-learned" type="checkbox" '+(!draft?'checked':'')+'>仅已学习</label></div><div id="character-skills"></div>'+(draft?'<div class="character-edit-footer"><span>正在编辑 · '+esc(c.name)+'</span><button id="character-discard">结束编辑</button><button id="character-save">保存修改</button></div>':'');
  $('#skill-search').oninput=drawSkills;$('#skills-learned').onchange=drawSkills;
  $('#character-avatar').onclick=()=>editPilotAvatar(c,()=>{drawList();$('#character-avatar').innerHTML=pilotPortrait(c);});
  if(draft){$('#skills-zero').onclick=()=>{draft.skills=[];markDirty();drawSkills()};for(let level=1;level<=5;level++){const b=document.createElement('button');b.textContent='全部 '+['','I','II','III','IV','V'][level];b.onclick=()=>setAllSkills(level);$('#skill-presets').append(b);}$('#character-discard').onclick=async()=>{if(!await guard())return;draft=null;drawDetail();status('')};$('#character-save').onclick=async()=>{const button=$('#character-save');button.disabled=true;try{const c=await api('character',draft);copyDraftAvatar(draft,c);people=people.filter(p=>p.id!==c.id).concat(c);if(!draft.id&&activeFolder)movePerson(c.id,activeFolder);selected=c;draft=null;dirty=false;cacheDraft();drawList();drawDetail();status('角色已保存')}catch(e){status(e.message)}finally{if(button.isConnected)button.disabled=false}}}
 ...
 let pasting=false;
 const clipboardScope=e=>!root.hidden&&!document.querySelector('dialog[open]')&&!e.target.closest?.('input,textarea,select,[contenteditable="true"]')&&!!e.target.closest?.('.character-roster');
 document.addEventListener('copy',e=>{
  if(!clipboardScope(e)||!selected||!e.clipboardData||window.getSelection()?.toString())return;
