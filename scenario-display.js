...
export function renderWorkspaceAttack(report,catalog){
 ...
 const edps=view.attackMode==='edps',damageUnit=edps?'EDPS':'DPS';
 ...
 const weaponTerms=report.scenarioAnalysis.weapons.map(r=>[
  ...
  metricText(view.target?r.appliedDpsWithoutReload:r.baseDps,damageUnit),null,
  scenarioDetail(name(r.typeId)||r.name,metricText(view.target?r.appliedDpsWithoutReload:r.baseDps,damageUnit),view.target?r.appliedDpsWithoutReload:r.baseDps,view.active?r.baseDps:undefined,
   view.target?[['射程与运动应用系数',metricText(r.factor)],['换弹折算系数',metricText(r.reloadFactor)],['包含目标抗性',edps?'是':'否']]:[],conditions)
 ]);
 function detail(key,title,unit,terms){
  return {...scenarioDetail(title,metricText(a[key],unit),a[key],comparison(key),terms,conditions,metricText(b[key],unit)),...(key==='total'?{chart:{...(view.curves||{status:'unavailable',reason:'曲线数据尚未就绪。'}),totalDps:b.total}}:{})};
 }
 ...
 let html=`<div class="panel-title"><span>攻击</span><span class="attack-modes defense-modes" role="group" aria-label="伤害显示模式"><button data-attack-mode="dps" aria-pressed="${!edps}">DPS</button><button data-attack-mode="edps" aria-pressed="${edps}" ${view.target?'':'disabled title="选择情景目标后查看 EDPS"'}>EDPS</button></span><span class="attack-total" data-workspace-metric="total" ${explanationAttributes(detail('total','总 '+damageUnit,damageUnit,weaponTerms.concat([['无人机',metricText(a.drone,damageUnit)]])))}><b ${numberAttributes(a.total,comparison('total'))}>${metricText(a.total,damageUnit,1)}</b></span></div><div class="stat-block">`;
 html+=row('weapon','武器 '+damageUnit,'',weaponTerms)+row('drone','无人机 '+damageUnit,'',[[view.target&&a.drone===null?'当前未支持':'范围',view.target&&a.drone===null?'缺少无人机自身运动与攻击应用模型':'已出动无人机']]);
 html+=row('reload','含换弹 '+damageUnit,'',report.scenarioAnalysis.weapons.map(r=>[(name(r.typeId)||r.name),metricText(view.target?r.appliedDps:r.reloadDps,damageUnit)]))+row('volley','齐射伤害 · DPH','HP',[[view.target?'口径':'范围',view.target?(edps?'目标抗性后的期望武器齐射伤害':'不扣抗性的期望武器齐射伤害'):'舰载武器齐射']]);
 if(a.profile){html+='<div class="damage-bars">'+['em','thermal','kinetic','explosive'].map((key,i)=>{const value=a.profile[key]??0,percent=a.total>0?value/a.total*100:0,base=b.profile?.[key],basePercent=b.total>0?(base??0)/b.total*100:0;return `<div class="damage-cell"><span class="damage-label">${['电磁','热能','动能','爆炸'][i]}</span><div class="mini-bar damage-${i}" role="progressbar" aria-label="${['电磁','热能','动能','爆炸'][i]}伤害占比" aria-valuenow="${percent}" aria-valuemin="0" aria-valuemax="100"><i style="width:${Math.max(0,Math.min(100,percent))}%"></i><b ${numberAttributes(percent,view.active?basePercent:undefined)}>${metricText(percent,'%',0)}</b></div><span class="damage-component"><b ${numberAttributes(value,view.active?base:undefined)}>${metricText(value,'',1)}</b><small>${damageUnit}</small></span></div>`}).join('')+'</div>';}
 if(view.target)html+='<p class="profile-note">'+(edps?'EDPS · 已扣目标抗性':'DPS · 不扣目标抗性')+' · 不含换弹</p>';
