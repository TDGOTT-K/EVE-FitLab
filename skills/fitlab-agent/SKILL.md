...
- Non-module fitting objects: read `fitting roster`, then use `setFighters`, `setDrones`, `setSubsystems`, `setImplants`, or `setBoosters` in preview/edit. These replace the entire named collection, so retain existing members when adding. Fighter entries need stable squadron/member IDs, typeId, deployed, and explicit location/tubeIndex when loaded. Query the actual size limit; do not assume nine fighters per squadron. Static fighter primary DPS is not combined rockets/utility/combat output. A category error from install is a routing error, not proof that the engine cannot fit fighters.
