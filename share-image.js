...
export async function renderShareImage(fit,report,catalog,price,options){
 ...
 if(report.scenarioLinks?.targetFitId){const t=report.scenarioAnalysis;y=wrap('靶标 '+report.scenarioLinks.targetFitId.name+' · '+number(t.target.distance)+' m · '+number(t.target.speed)+' m/s · '+number(t.target.angular)+' rad/s',48,y,984,20,'#e9b479');y=wrap('对目标期望武器 DPS（含换弹） '+number(t.appliedWeaponDps),48,y,984,20,'#e9b479');}
 for(const [key,label] of [['supportFitId','传电来源'],['hostileFitId','毁电来源']])if(report.scenarioLinks?.[key])y=wrap(label+'：'+report.scenarioLinks[key].name,48,y,984,20,'#e9b479');
...
export async function showShareImage(source,{calculate,catalog,getPrice}){
 ...
 dialog.querySelectorAll('input,select').forEach(e=>e.onchange=render);download.onclick=()=>{const a=document.createElement('a');a.href=url;a.download=(fit.name||'装配').replace(/[<>:"/\\|?*\x00-\x1f]/g,'_').slice(0,100)+'-EVE-FitLab.png';a.click()};copy.onclick=async()=>{try{await navigator.clipboard.write([new ClipboardItem({'image/png':blob})]);status.textContent='图片已复制，可以粘贴到聊天窗口'}catch{status.textContent='浏览器不支持复制或未授予权限，请保存 PNG'}};await render();
