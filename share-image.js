...
export async function renderShareImage(fit,report,catalog,price,options){
 if(report.provider==='nengine')return renderNativeShareImage(fit,report,catalog,options);
...
export async function showShareImage(source,{calculate,catalog,getPrice}){
 ...
 try{const report=await calculate(fit),price=report.provider==='nengine'?null:await getPrice(fit).catch(()=>null);if(!dialog.open)return;
