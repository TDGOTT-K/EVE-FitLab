...
function loadAll(t){const target=slots.filter(s=>acceptsAmmo(byId(s.item),t));if(!target.length){say('没有可使用这种弹药的已装备模块');return}mutate(()=>target.forEach(s=>{if(s.ammo!==t.id)delete s.loadedCharges;s.ammo=t.id}),`已为 ${target.length} 件兼容装备装填 ${t.name}`)}
...
async function editLoadedCount(key){
 ...
 try{
  ...
  const row=result.native?.inventory?.magazines.find(m=>m.moduleId===key);
  if(!row){say('引擎未提供可用弹仓容量');return;}
  openFlow('实际装弹量','<form id="magazine-form"><p>'+esc(byId(slot.ammo).name)+' · 容量 '+row.capacity+' 发</p><label>装弹数量 <input type="number" min="0" max="'+row.capacity+'" step="1" required value="'+(slot.loadedCharges??0)+'" aria-label="实际装弹数量"></label><p class="profile-note">声明当前弹仓；不会自动从货舱扣除。零表示空仓，未声明表示未知。</p><button type="submit">应用</button><button type="button" data-fill>满弹</button><button type="button" data-unknown>恢复未声明</button></form>');
  const form=$('#magazine-form'),input=form.querySelector('input');
  form.querySelector('[data-fill]').onclick=()=>{input.value=row.capacity};
  form.querySelector('[data-unknown]').onclick=()=>{if(version!==analysisVersion){$('#flow-error').textContent='装配已变化，请重新打开';return;}mutate(()=>delete slots.find(s=>s.key===key).loadedCharges,'装弹量恢复未声明');flow.close()};
  form.onsubmit=async e=>{e.preventDefault();const count=Number(input.value);if(!Number.isInteger(count)||count<0||count>row.capacity)return;
   const candidate=currentFit();candidate.slots.find(s=>s.key===key).loadedCharges=count;form.inert=true;
   try{const checked=await getCalculation(candidate);if(version!==analysisVersion)throw Error('装配已变化，请重新打开');const error=checked.issues.find(i=>i.code==='FIT_INVENTORY_MAGAZINE_OVERFLOW');if(error)throw Error(error.message);mutate(()=>slots.find(s=>s.key===key).loadedCharges=count,'已声明实际装弹量 · 可撤销');flow.close()}catch(error){$('#flow-error').textContent=error.message}finally{form.inert=false}
  };input.focus();input.select();
 }catch(error){say('弹仓不可用：'+error.message)}
}
