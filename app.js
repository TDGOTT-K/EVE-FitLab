...
function renderTree(){if(filter?.fighters&&fighterHull(ship)&&!abyssalLibrary.active){document.querySelector('#filter').innerHTML='<button>铁骑舰载机 ×</button>';document.querySelector('#filter button').onclick=()=>{filter=null;renderTree()};drawFighterBrowser(document.querySelector('#tree'),document.querySelector('#search').value.trim());return;}cancelInstallPreview();const root=$('#tree'),scroll=root.scrollTop,closed=new Set();const selected=filter&&slots.find(s=>s.key===filter.key);$('#filter').innerHTML=selected?`<button aria-label="清除装备筛选">${filter.ammo?'兼容弹药 · '+esc(byId(selected.item)?.name||''):labels[selected.kind]+' · 槽位 '+(Number(selected.key.split('-')[1])+1)}　×</button>`:'';if(selected)$('#filter button').onclick=()=>{filter=null;renderSlots();renderTree()};if(filter?.bay){$('#filter').innerHTML='<button aria-label="清除装备筛选">'+(filter.bay==='drones'?'无人机库':'货舱')+' ×</button>';$('#filter button').onclick=()=>{filter=null;renderSlots();renderTree()}}const q=$('#search').value.trim().toLowerCase();if(q!==treeSearchQuery){searchCollapsed.clear();treeSearchQuery=q}const items=catalog.filter(t=>!['ship','skill'].includes(t.kind)&&(!filter?.bay||acceptsBay(t,filter.bay))&&(t.kind!=='subsystem'||t.attrs[1380]===ship.id)&&(!selected||(filter.ammo?acceptsAmmo(byId(selected.item),t):canInstall(t,selected.key)))&&(!q||(matchesName(t,q)||t.path.some(p=>tLabel(p).toLowerCase().includes(q))||t.path.join(' ').toLowerCase().includes(q)||abyssalLibrary.matches(t,q))));if(abyssalLibrary.draw(items,q))return;$('#count').textContent=`${items.length} 件`;const tree={};for(const t of [...items].sort((a,b)=>(a.kind==='ammo')-(b.kind==='ammo'))){let n=tree;for(const p of [...t.path,...(t.kind==='ammo'?[]:[t.meta||'未标注科技分类'])]){n[p]??={};n=n[p]}(n._items??=[]).push(t)}
