...
async function install(t,key){
 if(!t)return;
 if(t.en?.includes('Abyssal')&&!t.abyssal){say('请从深渊库选择已生成的实例');return;}
...
function loadedCountText(slot){
 if(isCrystal(slot.ammo)){const c=fitRecord.crystals?.find(c=>c.moduleId===slot.key);return c?(c.damage===0?'100%':crystalWearText(reportVersion===analysisVersion?crystalProjection(report,c.id):null)):'—';}
...
const isCrystal=id=>[86,374,375].includes(byId(id)?.group);
async function fillNewAmmo(candidate,keys){
 ...
 for(const key of keys){const slot=candidate.slots.find(s=>s.key===key);if(!slot?.ammo||isCrystal(slot.ammo))continue;
...
async function editLoadedCount(key){
 ...
 try{
  if(isCrystal(slot.ammo)&&!candidate.crystals?.some(c=>c.moduleId===key))await fillNewAmmo(candidate,[key]);
  if(!isCrystal(slot.ammo))candidate.slots.find(s=>s.key===key).loadedCharges??=0;
  ...
  const capacity=isCrystal(slot.ammo)?100:checked.native?.inventory?.magazines.find(m=>m.moduleId===key)?.capacity;
  if(!Number.isFinite(capacity)||isCrystal(slot.ammo)&&!(hp>0))throw Error('引擎未提供可编辑的容量或耐久');
  const input=document.createElement('input');input.type='number';input.className='ammo-inline-input';input.min=isCrystal(slot.ammo)?'0.01':'0';input.max=capacity;input.step=isCrystal(slot.ammo)?'any':'1';
  input.setAttribute('aria-label',isCrystal(slot.ammo)?'晶体剩余耐久百分比':'实际装弹数量');input.value=isCrystal(slot.ammo)?Number((100*(1-crystal.damage/hp)).toFixed(4)):(slot.loadedCharges??capacity);
  anchor.replaceChildren(input);input.onclick=e=>e.stopPropagation();let done=false;
  const commit=()=>{if(done)return;const value=Number(input.value);if(!input.reportValidity()||!Number.isFinite(value)||!isCrystal(slot.ammo)&&!Number.isInteger(value))return;done=true;
   if(version!==analysisVersion){refreshSlotMetrics();return;}
   if(isCrystal(slot.ammo))crystal.damage=hp*(1-value/100);else candidate.slots.find(s=>s.key===key).loadedCharges=value;
