...
document.addEventListener('fitlab-loadout-menu',event=>{
 const {event:e,item,origin,entries}=event.detail;menuOrigin=origin;
 const menu=$('#menu');menu.innerHTML='<div class="menu-title">'+esc(item.name)+'</div>';
 const actions=[...entries,['详细信息',()=>showInfo({...item,kind:'loadout',path:['脑插与增效剂',...(item.benefitLabels||[])],attrs:{}})]];
 for(const [label,fn] of actions){const b=document.createElement('button');b.type='button';b.role='menuitem';b.innerHTML=menuIcon(label==='详细信息'?'查看信息':label==='卸下'?'卸载装备':label)+'<span class="menu-action-label">'+esc(label)+'</span>';if(label==='卸下')b.className='danger';b.onclick=()=>{closeMenu();fn()};menu.append(b);}
 menu.hidden=false;const r=origin.getBoundingClientRect(),keyboard=e.type==='keydown';menu.style.left=Math.max(8,Math.min(keyboard?r.left:e.clientX,innerWidth-menu.offsetWidth-8))+'px';menu.style.top=Math.max(8,Math.min(keyboard?r.bottom:e.clientY,innerHeight-menu.offsetHeight-8))+'px';menu.querySelector('button')?.focus({preventScroll:true});
});
