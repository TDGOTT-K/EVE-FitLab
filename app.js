...
function refreshEditView(){updateShip();save();renderSlots();if(!report){renderResources();renderShipStats()}renderTree();$('#undo').disabled=!history.length;$('#redo').disabled=!redoHistory.length;}
...
function refreshSlotMetrics(){document.querySelectorAll('[data-module-metrics]').forEach(el=>{const s=slots.find(s=>s.key===el.dataset.moduleMetrics);if(s)el.innerHTML=slotMetrics(s,el.dataset.metricGroup||'details')});document.querySelectorAll('[data-loaded-count]').forEach(b=>{const s=slots.find(s=>s.key===b.dataset.loadedCount);if(s&&!b.querySelector('input'))b.textContent=loadedCountText(s)})}
function renderSlots(){const root=$('#slots'),scroll=$('.fitting').scrollTop;renderImplantEntry();renderBayConfig();root.innerHTML=Object.keys(counts).filter(kind=>counts[kind]||slots.some(s=>s.kind===kind&&s.item)).map(kind=>`<div class="slot-heading"><span>${labels[kind]}</span><span class="slot-limits" data-rack-limits="${kind}">${rackLimits(kind)}</span></div>`+slots.filter(s=>s.kind===kind).map(s=>{const t=byId(s.item),a=byId(s.ammo);return `<div class="slot ${t?'filled':''} ${filter?.key===s.key&&!filter.ammo?'selected':''} ${moduleState(s)==='Offline'?'offline':''} state-${moduleState(s).toLowerCase()}" data-key="${s.key}"><div class="module" tabindex="0" role="button" aria-label="${labels[kind]} ${Number(s.key.split('-')[1])+1} ${t?esc(s.abyssalName||t.name):'空槽'}">${t?img(t):'<span class="empty-symbol">＋</span>'}<span class="name ${t?'':'empty-text'}"><span class="module-heading"><span class="module-identity">${t?esc(s.abyssalName||t.name):kind==='subsystem'?['核心子系统','防御子系统','攻击子系统','推进子系统'][Number(s.key.split('-')[1])]:labels[kind]}${t?`<small>${kind==='rig'?`校准 ${t.attrs[1153]||0} · 尺寸 ${t.attrs[1547]}`:`${moduleState(s)==='Online'&&t.canActivate===false?'在线 · 被动':stateLabels[moduleState(s)]}`}</small>`:''}</span>${t&&kind!=='rig'?`<span class="slot-resources" data-module-metrics="${s.key}" data-metric-group="resources">${slotMetrics(s,'resources')}</span>`:''}</span>${t&&kind!=='rig'?`<span class="slot-metrics" data-module-metrics="${s.key}">${slotMetrics(s)}</span>`:''}</span></div>${needsAmmo(t)?`<div class="ammo ${filter?.key===s.key&&filter.ammo?'selected':''}" tabindex="0" role="button" aria-label="${esc(t.name)}的弹药槽">${a?img(a):'<span>↳</span>'}<span>${a?esc(a.name):'弹药槽 · 点击筛选 / 拖入装填'}</span>${a?`<span class="ammo-count" role="button" tabindex="0" data-loaded-count="${s.key}" aria-label="编辑弹药数量或晶体耐久">${loadedCountText(s)}</span>`:''}</div>`:''}</div>`}).join('')).join('');$('.fitting').scrollTop=scroll;
...
async function install(t,key){
 ...
 if(report?.provider==='nengine'){
  ...
  try{const candidate=await installCandidate(currentFit(),t,key);const checked=await getCalculation(candidate);if(version!==analysisVersion){say('装配已变化，请重新安装');return;}const blocked=checked.issues.filter(e=>['HARDPOINT_LIMIT','QUANTITY_LIMIT','ACTIVE_GROUP_LIMIT','SLOT_UNAVAILABLE','SLOT_OCCUPIED','SLOT_UNSUPPORTED'].includes(e.code));if(blocked.length){say('无法安装：'+blocked.map(e=>e.code+' · '+e.message).join('；'));return;}mutate(()=>{slots=candidate.slots;fitRecord.crystals=candidate.crystals},`已${t.kind==='ammo'?'装填':'安装'} ${t.name}`);return;}catch(e){say('安装校验失败：'+e.message);return;}
 }
