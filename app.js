...
function applyCandidate(slot,item){
 ...
 delete slot.mutation;delete slot.abyssalName;delete slot.abyssalInstanceId;
...
function openBatchMenu(e,keys,title){
 ...
 actions.push(['卸载装备',()=>mutate(()=>targets().forEach(s=>{s.item=null;s.ammo=null;delete s.mutation;delete s.abyssalName;delete s.abyssalInstanceId}),'已批量卸载装备'),targets().length>0],['卸载弹药',()=>mutate(()=>targets().forEach(s=>s.ammo=null),'已批量卸载弹药'),targets().some(s=>s.ammo)]);
