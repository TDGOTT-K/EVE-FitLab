...
export function mountEngineStats(root,report,ship,pilot,mode,onToggle,catalog=[],profile,onEdit){
 ...
 const tip=(title,result,terms)=>`tabindex="0" data-explain="${esc(JSON.stringify(calculationDetail(a.calculationDetails?.[({'DPS':'appliedDamagePerSecond','无人机 DPS':'appliedDroneDamagePerSecond','模块耗电':'capacitorUsagePerSecond'})[title]],catalog,pilot)||{title,result,terms,conditions:context}))}"`;
 ...
 const row=(name,value,terms)=>`<div class="stat-row" ${unchanged(name,value)?'':tip(name,value,terms)}><span>${name}</span><b>${value}</b></div>`;
 ...
 if(snapshot.droneControlDistance>0)root.insertAdjacentHTML('beforeend',head('无人机')+'<div class="stat-block">'+extraAttribute('控制距离','droneControlDistance','km',1000)+extraAttribute('最大操控数量','maxActiveDrones','架')+'</div>');
 ...
 const body=[['持续回盾','shield',layers[0]],['持续修甲','armor',layers[1]],['持续修结构','structure',layers[2]]].filter(([n,k])=>sustained[k]>0).map(([n,k,l])=>row(n,defenseNumber(ehp?effectiveValue(sustained[k],l.mean):sustained[k],2)+(ehp?' EHP/s':' HP/s'),[['预热','20 分钟'],['统计窗口','随后 10 分钟'],['调度规则','按装配顺序支付耗电；失败后下个周期重试'],['来伤分布',ehp?distribution:'不按抗性折算']])).join('');
