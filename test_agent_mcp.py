 ...
 def test_public_workflow_and_bounded_references(self):
  ...
  with tempfile.TemporaryDirectory() as state:
   ...
   try:
    ...
    frozen=json.loads(Path('contracts/agent-mcp-v1/tools.json').read_text(encoding='utf-8'))
    self.assertEqual(frozen['tools'],TOOLS)
    status=agent.call('fitlab_status',{});self.assertEqual(status['contractRevision'],59)
