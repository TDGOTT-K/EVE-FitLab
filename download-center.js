...
export function installDownloadCenter(){
 ...
 document.body.append(popup);entry.onclick=()=>{const rect=entry.getBoundingClientRect();popup.style.right=Math.max(12,innerWidth-rect.right)+'px';popup.style.top=rect.bottom+12+'px';popup.togglePopover();};popup.addEventListener('toggle',e=>entry.setAttribute('aria-expanded',String(e.newState==='open')));
