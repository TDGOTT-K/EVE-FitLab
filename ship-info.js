export function shipInfoMarkup(data,catalog,{esc,local,number}){
 ...
 for(const trait of data.shipTraits?.items||[]){
  ...
  const title=trait.kind==='skill_per_level'?(skill?.name||'技能 #'+trait.skillTypeId)+' · 每级收益':trait.kind==='role'?'船体特有加成':'其他特性';
 ...
 const traits=[...groups].map(([title,items])=>'<section class="ship-info-group"><h3>'+esc(title)+'</h3><ul>'+items.join('')+'</ul></section>').join('')||'<p class="profile-note">当前来源未提供舰船特性。</p>';
 ...
 return '<div data-info-pane="traits">'+traits+'<p class="profile-note">舰船特性说明 · 加成是否已计算以装配结果为准</p></div><div data-info-pane="requirements"><h3>驾驶所需技能</h3>'+required+'<p class="profile-note">舰船直接前置要求；不代表当前角色已满足全部技能链。</p></div>';
}
