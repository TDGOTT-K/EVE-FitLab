"""Build UI benefit taxonomy from bundled SDE attributes, without engine formulas."""
import json
from pathlib import Path
ROOT=Path(__file__).resolve().parent.parent
BASE=ROOT/'desktop/build/sde'
def rows(name):return map(json.loads,(BASE/(name+'.jsonl')).read_text(encoding='utf-8').splitlines())
dogma={t['_key']:t for t in rows('typeDogma')}
attributes={t['_key']:t for t in rows('dogmaAttributes')}
MAP={}
...
# Generic attributes are interpreted using their actual item descriptions.
TYPES={t['_key']:t for t in rows('types')}
unknown={}
for filename in ['implant-catalog.js','booster-catalog.js']:
 p=ROOT/filename;s=p.read_text(encoding='utf-8');items=json.loads(s[s.index('['):].rstrip(';\n'))
 for t in items:
  ...
  for id,value in a.items():
   ...
   else:unknown.setdefault(str(id)+' '+name,[]).append(t['id']);tags.append(('训练与其他','其他特殊效果'))
  ...
  if filename.startswith('implant') and pure:tags=[('训练与其他','白板')]
...
print('Unmapped attributes:',unknown)
