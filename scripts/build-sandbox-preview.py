...
parser=argparse.ArgumentParser();parser.add_argument('--engine',required=True);parser.add_argument('--evidence',required=True);args=parser.parse_args()
...
run('run','--scenario',p/'scenario.json','--policy',p/'policy.js','--ticks',1,'--out',p/'frame-01')
for second in range(2,41):run('step','--checkpoint',p/f'frame-{second-1:02d}'/'checkpoint.json','--ticks',1,'--out',p/f'frame-{second:02d}')
frames=[];events=[]
...
for sec in range(1,41):
 d=p/f'frame-{sec:02d}';cp=json.loads((d/'checkpoint.json').read_text(encoding='utf-8-sig'));w=cp['world'];frames.append({'time':sec,'ships':[ship(s) for s in w['ships']]})
...
run('run','--scenario',p/'scenario.json','--policy',p/'policy.js','--ticks',40,'--out',p/'continuous')
...
effects=[{k:e[k] for k in ['sourceId','targetId','abilityId','startedAtUs','expiresAtUs','endedAtUs']} for e in w['effects']]
missiles=[{k:m[k] for k in ['id','sourceId','targetId','launchedAtUs','expiresAtUs','launchPosition']} for m in w['missiles']]
meta=[{'id':r['id'],'team':r['team'],'typeId':r['fit']['shipTypeId']} for r in request['participants']]
result={'version':1,'duration':40,'engineVersion':cp['engineVersion'],'source':'SDE 3503375 / native battle assembly','scope':'experimental recorded battle; no full combat certification','seed':17,'ships':meta,'frames':frames,'events':[{k:e.get(k) for k in ['sequence','timeUs','kind','sourceId','targetId','reason','causeId','entityId']} for e in events if e['kind'] in ['TurretResolved','MissileLaunched','MissileImpacted','MissileExpired','ShipDestroyed','EffectStarted','EffectEnded','DamageApplied']],'effects':effects,'missiles':missiles,'script':(p/'policy.js').read_text()}
