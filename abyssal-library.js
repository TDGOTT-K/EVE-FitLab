...
export function installAbyssalLibrary({host,catalog,api,render,onLocate,onInfo}){
 ...
 function edit(t,record=null,copy=false){
  ...
  openAbyssalWorkbench({type:t,record,copy,onSave:async fields=>{const saved=await api('abyssal-instance',{...(current?{id:current.id,revision:current.revision}:{}),baseTypeId:t.id,...fields});records=records.filter(x=>x.id!==saved.id).concat(saved);loaded=true;reveal(t);search.value='';render();}});
 }
